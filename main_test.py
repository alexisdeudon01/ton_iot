...
class DetailedTestPlugin:
    ...
    def pytest_fixture_setup(self, fixturedef, request):
        ...
        try:
            ...
            if nodeid not in self.test_fixtures:
                self.test_fixtures[nodeid] = {}
        except Exception:
            pass
    ...
    def pytest_runtest_logreport(self, report):
        ...
        if report.when == "call":  # Only capture actual test execution
            ...
            if outcome == "passed":
                result.success_reason = self._generate_detailed_success_reason(result, test_name, "", method_desc)
    ...
    def _analyze_test_inputs(self, item):
        ...
        try:
            ...
            if item.nodeid in self.test_source_code:
                matrices = self._extract_input_matrices_from_code(item.nodeid, docstring)
                if matrices:
    ...
    def _extract_input_matrices_from_code(self, nodeid: str, docstring: str) -> List[MatrixInfo]:
            ...
            for i, line in enumerate(lines):
                ...
                if in_input_section and line.strip():
                    ...
                    if 'dataframe' in line_lower or 'array' in line_lower or 'matrix' in line_lower:
                        ...
                        fusion_info = None
                        # (Fusion info extraction logic could be added here)
        ...
        if not matrices and nodeid in self.test_source_code:
            source = self.test_source_code[nodeid]
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Assert):
                    ...
                    if condition_source:
                        ...
                        description = f"Assertion: {condition_source[:80]}"
    ...
    def _generate_detailed_success_reason(self, result: TestResult, test_name: str, docstring: str, method_desc: str) -> str:
...
def generate_test_coverage_diagram(results: List[TestResult], output_dir: Path) -> Optional[Path]:
...
def generate_detailed_report(results: List[TestResult], output_dir: Path) -> Path:
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        if total > 0:
            ...
            f.write("- **Passed**: 0 (0.0%)\n")
            f.write("- **Failed**: 0 (0.0%)\n")
            f.write("- **Skipped**: 0 (0.0%)\n\n")
