...
class T13_TrainTON(Task):
    def run(self, context: DAGContext) -> TaskResult:
        ...
        for model_type in algos:
            ...
            output_path = os.path.join(context.config.paths.work_dir, "models", f"ton_{model_type}.model")
            os.makedirs(os.path.dirname(output_path), exist_ok=True)

            if model_type in ["LR", "DT", "RF"]:
                model = SklearnModel(model_type, ton_art.feature_order)
            elif model_type == "CNN":
                model = TorchCNNModel(ton_art.feature_order, epochs=2 if context.config.test_mode else 10)
            ...
            try:
                ...
                context.logger.info("training", f"Model TON {model_type} saved in {duration:.2f}s")
