...
class DatasetLoader:
    ...
    def _analyze_csv_headers(self, file_path: Path, n_rows: int = 1000) -> Tuple[List[str], pd.DataFrame]:
        ...
        try:
            ...
            sample_df = pd.read_csv(file_path, nrows=n_rows, low_memory=False, encoding='utf-8-sig')
    ...
    def load_ton_iot(self, file_path: Optional[str] = None, sample_ratio: float = 1.0,
        ...
        try:
            ...
            if sample_ratio < 1.0:
                if sample_ratio < 0.01:  # For very small ratios (< 1%), use decimation
                    ...
                    df = pd.read_csv(file_path_obj, skiprows=skip_func, low_memory=False,
                                   nrows=int(1e6), encoding='utf-8-sig')  # Limit to 1M rows max for safety
            ...
            chunk_iterator = pd.read_csv(file_path, low_memory=False, chunksize=chunk_size, encoding='utf-8-sig')
            ...
            try:
                ...
                if sample_ratio < 1.0:
                    if sample_ratio < 0.01:  # For very small ratios (< 1%), use decimation
                        ...
                        df_file = pd.read_csv(csv_file, skiprows=skip_func, low_memory=True, nrows=int(1e6), encoding='utf-8-sig')
...
- [Pylance Error] Line 54: Argument of type "_T_co@tuple" cannot be assigned to parameter "x" of type "ConvertibleToFloat" in function "__new__"
  Type "_T_co@tuple" is not assignable to type "ConvertibleToFloat"
...
- [Pylance Error] Line 55: Argument of type "_T_co@tuple" cannot be assigned to parameter "x" of type "ConvertibleToFloat" in function "__new__"
  Type "_T_co@tuple" is not assignable to type "ConvertibleToFloat"
    "object*" is not assignable to "str"
    "object*" is incompatible with protocol "Buffer"
      "__buffer__" is not present
...
    "object*" is incompatible with protocol "SupportsIndex"
      "__index__" is not present
- [Pylance Error] Line 539: Argument of type "_T_co@tuple" cannot be assigned to parameter "x" of type "ConvertibleToFloat" in function "__new__"
  Type "_T_co@tuple" is not assignable to type "ConvertibleToFloat"
    "object*" is not assignable to "str"
    "object*" is incompatible with protocol "Buffer"
      "__buffer__" is not present
...
    "object*" is incompatible with protocol "SupportsIndex"
      "__index__" is not present
- [Pylance Error] Line 540: Argument of type "_T_co@tuple" cannot be assigned to parameter "x" of type "ConvertibleToFloat" in function "__new__"
  Type "_T_co@tuple" is not assignable to type "ConvertibleToFloat"
    "object*" is not assignable to "str"
    "object*" is incompatible with protocol "Buffer"
      "__buffer__" is not present
...
../../.github/copilot-instructions.md
../../_old/legacy_scripts/RL_training.py
../../_old/legacy_scripts/RL_training.py
../../_old/legacy_scripts/data_training.py
../../_old/legacy_scripts/data_training.py
../../output/phase1_config_search/best_config.json
../../output/phase1_config_search/best_config.json
src/core/dataset_loader.py

# VSCode Open Tabs
src/phases/phase1_config_search.py
tests/test_evaluation_3d_comprehensive.py
...
# Current Time
1/17/2026, 8:08:49 PM (Europe/Luxembourg, UTC+1:00)

# Context Window Usage
410,442 / 1,048.576K tokens used (39%)
