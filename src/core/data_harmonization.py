...
class DataHarmonizer:
    ...
    # Thresholds for statistical compatibility
    KS_PVALUE_THRESHOLD = 0.05
    RELATIVE_MEAN_DIFF_THRESHOLD = 0.1
    ...
    def analyze_feature_similarity(self, df1: pd.DataFrame, df2: pd.DataFrame,
        ...
        s1, s2 = val1.std(), val2.std()

        mean_diff = abs(m1 - m2)
        std_diff = abs(s1 - s2)

        # Consider compatible if:
        # 1. KS test fails to reject null hypothesis (distributions are statistically similar)
        # 2. OR the relative difference between means is small (practically similar)
        avg_mean = (m1 + m2) / 2

        if avg_mean != 0:
            practically_similar = (mean_diff / avg_mean) < self.RELATIVE_MEAN_DIFF_THRESHOLD
        else:
            # If both means are zero, they are perfectly similar
            practically_similar = (mean_diff == 0)

        compatible = bool((ks_pvalue > self.KS_PVALUE_THRESHOLD) or practically_similar)
