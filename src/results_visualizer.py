...
class ResultsVisualizer:
    ...
    def _create_algorithm_viz_tab(self, frame, row, model_name: str):
        ...
        if exp_components:
            ax3.bar(exp_components, exp_values, color=plt.cm.viridis(np.linspace(0, 1, len(exp_components))))
    ...
    def _create_radar_chart(self, frame):
        ...
        if dim_scores_path.exists():
            ...
            dim_scores['detection_performance'] = dim_scores['f1_score']
            dim_scores['resource_efficiency'] = 1 - (dim_scores['training_time_seconds'] / dim_scores['training_time_seconds'].max())
            dim_scores['explainability'] = dim_scores['explainability_score']
