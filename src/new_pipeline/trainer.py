...
class LateFusionTrainer:
    ...
    def train(self, train_path: Path, val_path: Path, model_name: ModelName, features: List[str]) -> TrainingResult:
        ...
        # Check for unimplemented models to avoid misleading artifacts
        if model_name in ["CNN", "TabNet"]:
            msg = f"Model {model_name} is not yet implemented in LateFusionTrainer. Please use LR, DT, or RF."
            logger.error(msg)
        ...
        try:
            ...
            if model_name == "LR":
                ...
                raise ModelTrainingError(model_name, ValueError(f"Unsupported model type: {model_name}"))
