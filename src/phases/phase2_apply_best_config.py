...
class Phase2ApplyBestConfig:
    ...
    def run(self) -> Dict:
        ...
        logger.info(f"Processed dataset: {df_final.shape[0]} rows, {df_final.shape[1]-1} features + label")
    ...
    def _apply_stateless_preprocessing(self, X: pd.DataFrame, y: pd.Series) -> tuple:
        ...
        logger.info("Applying stateless preprocessing (clean_data + encode_features only)...")
        logger.info("NOTE: Fit-dependent steps (scaling, feature selection, SMOTE) will be applied in Phase 3 per fold.")
        ...
        # Step 1: Clean data (NaN/Infinity removal, imputation)
        X_cleaned, y_cleaned = pipeline.clean_data(X, y)
        ...
        logger.info(f"Stateless preprocessing complete: {X_encoded.shape[0]} rows, {X_encoded.shape[1]} features")
        logger.info(f"Label distribution: {y_cleaned.value_counts().to_dict()}")
        
        return X_encoded, y_cleaned
    
    def _save_outputs(self, df_final: pd.DataFrame, feature_names: list) -> Dict[str, Path]:
    ...
    def _generate_summary(self, df_final: pd.DataFrame, feature_names: list) -> str:
        ...
        if 'dataset_source' in df_final.columns:
            ...
            lines.append(f"- **CIC-DDoS2019** (dataset_source=0): {ds_counts.get(0, 0):,} rows")
            lines.append(f"- **TON_IoT** (dataset_source=1): {ds_counts.get(1, 0):,} rows")
            lines.append("")
            lines.append("**Mapping**: `dataset_source` encoding: 0=CIC-DDoS2019, 1=TON_IoT")
            lines.append("")
        ...
        lines.append("- ❌ Feature selection (NOT applied - will be done in Phase 3 per fold)")
        lines.append("- ❌ Scaling (NOT applied - will be done in Phase 3 per fold)")
        lines.append("- ❌ SMOTE resampling (NOT applied - will be done in Phase 3 per fold)")
        lines.append("")
        lines.append("**Note**: Fit-dependent steps (scaling, feature selection, SMOTE) are applied in Phase 3 per fold to ensure zero data leakage.")
        lines.append("")
        ...
        if self.best_config:
            ...
            lines.append("```")
            lines.append("")
            lines.append("*Note: This configuration is documented for traceability. Only stateless steps are applied in Phase 2.*")
            lines.append("")
        ...
        lines.append(f"- `best_preprocessed.parquet` (or `.csv.gz`): Preprocessed dataset")
